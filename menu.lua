...
-- VERSION COMPLÈTE POUR DELTA MOBILE
-- FLY RÉPARÉ + MOBILE OPTIMISÉ | FLING 100% | WEBHOOK TOTAL
-- ══════════════════════════════════════════════════════

print("=== LULUCLC3 ZENITH v49.0 - DÉBUT CHARGEMENT DELTA MOBILE ===")
...
if not success or not Rayfield then
    warn("❌ Rayfield n'a pas chargé ! Essaie de relancer Delta ou vérifie ta connexion.")
    return
end

print("✅ Rayfield chargé avec succès sur Delta")

local Window = Rayfield:CreateWindow({
    Name = "LULUCLC3 ZENITH 👑 v49.0",
    LoadingTitle = "Chargement sur Mobile...",
    LoadingSubtitle = "par Luluclc3 • Delta Edition 2026",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "Luluclc3Zenith",
        FileName = "Ultimate_v49_Delta"
    },
    KeySystem = false
})
...
local isMobile = true -- Force mobile pour Delta
print("📱 Mode Delta Mobile activé")
...
SendWebhookLog("NOUVEAU LANCEMENT", "**Script complet chargé sur Delta Mobile**", 65280)
...
local function ToggleFly(state)
    ...
    if state then
        ...
        SendWebhookLog("🚀 FLY ACTIVÉ", "Mobile Delta OK", 65280)
    else
...
Home:CreateParagraph({Title = "Delta Mobile", Content = "Tout fonctionne sur mobile ! FLY + FLING + Webhook"})
...
Movement:CreateToggle({Name = "🚀 Fly (Mobile Delta)", CurrentValue = false, Callback = function(v)
    Settings.fly = v
    ToggleFly(v)
end})
...
Visuals:CreateToggle({Name = "ESP Complet", CurrentValue = false, Callback = function(v)
    Settings.esp = v
...
local executorInput = ServerTab:CreateInput({Name = "Colle ton code Lua ici", PlaceholderText = "-- Ton script ici", RemoveTextAfterFocusLost = false, Callback = function() end})
ServerTab:CreateButton({Name = "🚀 Exécuter", Callback = function()
    local code = executorInput.CurrentValue or ""
    if code ~= "" then
        SendWebhookLog("💻 Code Exécuté", "Longueur : " .. #code, 16776960)
        pcall(loadstring(code))
    end
end})

-- ==================== BOUCLE PRINCIPALE (optimisée mobile) ====================
...
Rayfield:Notify({
    ...
    Content = "Delta Mobile OK • FLY + FLING + Webhook tout logué",
    Duration = 8
})

print("=== SCRIPT COMPLÈTEMENT CHARGÉ SUR DELTA MOBILE ===")
    if state then
        hum.PlatformStand = true
        FlyBodyVelocity = Instance.new("BodyVelocity")
