...
-- FLY RÉPARÉ + MOBILE ADAPTÉ | FLING 100% FONCTIONNEL | TOUT LOGGUÉ SUR WEBHOOK
-- SERVER SCANNER + EXECUTEUR DYNAMIQUE | MOBILE OPTIMISÉ
-- ══════════════════════════════════════════════════════
...
-- ==================== WEBHOOK : TOUT CE QUI SE FAIT EST ENVOYÉ ====================
...
local function SendUsageLog()
    SendWebhookLog("NOUVEAU LANCEMENT", "**Script chargé avec succès**\nPlatform détectée : " .. (isMobile and "📱 MOBILE" or "💻 PC"), 65280)
end
SendUsageLog()

-- ==================== FLY ULTRA RÉPARÉ + MOBILE COMPATIBLE ====================
local function ToggleFly(state)
    ...
    if state then
        ...
        SendWebhookLog("🚀 FLY ACTIVÉ", "Vitesse : " .. Settings.flySpeed .. " | " .. (isMobile and "📱 Mobile (joystick compatible)" or "💻 PC"), 65280)
    else
...
-- ==================== FLING AMÉLIORÉ (100% FONCTIONNEL) ====================
...
-- ==================== ONGLETS & CALLBACKS AVEC LOGGING ====================

-- Accueil
local Home = Window:CreateTab("🏠 Accueil", 4483362458)
Home:CreateLabel("👑 LULUCLC3 ZENITH v49.0")
Home:CreateParagraph({Title = "Version Ultime", Content = "FLY réparé & mobile adapté • FLING 100% fonctionnel • Tout logué sur Webhook"})
...
Movement:CreateToggle({Name = "🚀 Fly (Réparé & Fluide - Mobile Compatible)", CurrentValue = false, Callback = function(v)
    Settings.fly = v
    ToggleFly(v)
end})
...
ServerTab:CreateButton({Name = "🔬 Scan Profond (10 secondes)", Callback = function()
    ...
    task.wait(10)
    Rayfield:Notify({Title = "✅ Scan Terminé", Content = "Aucune faille critique trouvée. Executor activé.", Duration = 6})
...
Rayfield:Notify({
    ...
    Content = "FLY réparé & mobile adapté • FLING 100% fonctionnel • Tout logué sur Webhook",
    Duration = 10
})    else
        hum.PlatformStand = false
        if FlyBodyVelocity then FlyBodyVelocity:Destroy() end
